/*
 * Copyright (C) 2010 HTC Corporation.
 *
 * This software is licensed under the terms of the GNU General Public
 * License version 2, as published by the Free Software Foundation, and
 * may be copied, distributed, and modified under those terms.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
*/

#include <linux/kernel.h>
#include <linux/init.h>
#include <linux/platform_device.h>
#include <linux/delay.h>
#include <linux/leds.h>
#include <linux/clk.h>
#include <linux/err.h>

#include <asm/io.h>
#include <asm/gpio.h>
#include <asm/mach-types.h>

#include <mach/msm_panel.h>
#include <mach/msm_fb.h>
#include <mach/vreg.h>
#ifdef CONFIG_HTC_PWRSINK
#include <mach/htc_pwrsink.h>
#endif

#include "board-beect.h"
#include "proc_comm.h"
#include "devices.h"

#if 0
#define B(s...) printk(s)
#else
#define B(s...) do {} while(0)
#endif

struct mddi_table {
	uint32_t reg;
	uint32_t value;
	uint32_t msec;
};

static struct mddi_table beect_init_tb[] = {
	{0x01,	0x00, 0},
	{0x3E,	0x3C, 0},
	{0x16,	0x08, 0},
	{0x21,	0x00, 0},
	{0x23,	0xA3, 0},
	{0x27,	0x02, 0},
	{0x28,	0x07, 0},
	{0x40,	0x74, 0},
	{0x41,	0xF0, 0},
	{0x71,	0x68, 0},
	{0x60,	0x01, 0},
	{0x93,	0x0F, 0},
	{0x98,	0x2F, 0},
	{0x9A,	0x00, 10},
	{0x1B,	0x10, 40},
	{0x1C,	0x04, 10},
	{0x1F,	0x09, 10},
	{0x43,	0x80, 60},
	{0xD0,	0x00, 0},
	{0xD1,	0x01, 0},
	{0xD2,	0x01, 0},
	{0xD3,	0x05, 0},
	{0xD4,	0x14, 0},
	{0xD5,	0x3F, 0},
	{0xD6,	0x01, 0},
	{0xD7,	0x09, 0},
	{0xD8,	0x01, 0},
	{0xD9,	0x0B, 0},
	{0xDA,	0x11, 0},
	{0xDB,	0x16, 0},
	{0xDC,	0x17, 0},
	{0xDD,	0x16, 0},
	{0xDE,	0x17, 0},
	{0xDF,	0x0C, 0},
	{0xE0,	0x10, 0},
	{0xE1,	0x00, 0},
	{0xE2,	0x2B, 0},
	{0xE3,	0x3A, 0},
	{0xE4,	0x3E, 0},
	{0xE5,	0x3E, 0},
	{0xE6,	0x3F, 0},
	{0xE7,	0x4E, 0},
	{0xE8,	0x55, 0},
	{0xE9,	0x0F, 0},
	{0xEA,	0x13, 0},
	{0xEB,	0x08, 0},
	{0xEC,	0x0A, 0},
	{0xED,	0x09, 0},
	{0xEE,	0x09, 0},
	{0xEF,	0x0F, 0},
	{0xF0,	0x15, 0},
	{0xF1,	0x1E, 0},
	{0xF2,	0x0F, 0},
	{0x5F,	0x01, 0},
	{0x96,	0x00, 1},
	{0x02,	0x00, 0},
	{0x03,	0x00, 0},
	{0x04,	0x00, 0},
	{0x05,	0xEF, 0},
	{0x06,	0x00, 0},
	{0x07,	0x00, 0},
	{0x08,	0x01, 0},
	{0x09,	0x3F, 0},
	{0x26,	0x24, 1},
	{0x26,	0x38, 1},
	{0x26,	0x3C, 0},
};

static struct mddi_table beect_standby_tb[] = {
	{0x26, 0x0038, 50},
	{0x26, 0x0020, 50},
};

struct lcm_va_cmd beect_gamma_init_seq[] = {
CMD_VECT(0x5e, 0,

),
};

struct lcm_va_cmd beect_auo_init_seq[] = {
        CMD_VECT(0xf0,  0, 0x5a, 0x00, 0x00, 0x00),
        CMD_VECT(0xf3,  0, 0x00, 0x00, 0x00, 0x00),
        CMD_VECT(0xf1,  0, 0x5a, 0x00, 0x00, 0x00),
        CMD_VECT(0xff,  0, 0x00, 0x00, 0x00, 0x40),
        CMD_VECT(0x11, 10, 0x00, 0x00, 0x00, 0x00),

        CMD_VECT(0xfd,  0, 0x11, 0x02, 0x00, 0x00),
        CMD_VECT(0xf3,  0, 0x01, 0x00, 0x00, 0x07, 0x33, 0x75, 0x66, 0x2c),
        CMD_VECT(0xf4,  0, 0x6B, 0x6B, 0x6C, 0x6C, 0x44, 0x00, 0x00, 0x00),
        CMD_VECT(0xf5, 10, 0x12, 0x00, 0x05, 0x00, 0x00, 0x1f, 0x00, 0x00),
        CMD_VECT(0xf3, 20, 0x03, 0x00, 0x00, 0x07, 0x33, 0x75, 0x66, 0x2c),
        CMD_VECT(0xff, 10, 0x00, 0x00, 0x00, 0x60),

        CMD_VECT(0xf3, 10, 0x07, 0x00, 0x00, 0x07, 0x33, 0x75, 0x66, 0x2c),
        CMD_VECT(0xff, 10, 0x00, 0x00, 0x00, 0x70),
        CMD_VECT(0xf3,  0, 0x0f, 0x00, 0x00, 0x07, 0x33, 0x75, 0x66, 0x2c),
        CMD_VECT(0xff, 10, 0x00, 0x00, 0x00, 0x78),
        CMD_VECT(0xf3, 20, 0x1f, 0x00, 0x00, 0x07, 0x33, 0x75, 0x66, 0x2c),

        CMD_VECT(0xf3, 30, 0x3f, 0x00, 0x00, 0x07, 0x33, 0x75, 0x66, 0x2c),
        CMD_VECT(0xf6,  0, 0x00, 0x08, 0x00, 0x00),
        CMD_VECT(0x2a,  0, 0x00, 0x00, 0x00, 0xef),
        CMD_VECT(0x2b,  0, 0x00, 0x00, 0x01, 0x3f),

        CMD_VECT(0xf7, 0, 0x80, 0x0b, 0x12, 0x30, 0x3a, 0x39, 0x3c, 0x3e,
                        0x00, 0x05, 0x20, 0x10, 0x0b, 0x3a, 0x10, 0x00),
        CMD_VECT(0xf8, 30, 0x80, 0x0b, 0x12, 0x30, 0x3a, 0x39, 0x3c, 0x3e,
                        0x00, 0x05, 0x20, 0x10, 0x0b, 0x3a, 0x10, 0x00),
        CMD_VECT(0xf9, 30, 0x80, 0x0b, 0x12, 0x30, 0x38, 0x3a, 0x3b, 0x3f,
                        0x03, 0x06, 0x1a, 0x10, 0x05, 0x3a, 0x10, 0x00),
        CMD_VECT(0xfa, 20, 0x80, 0x0b, 0x12, 0x30, 0x38, 0x3a, 0x3b, 0x3f,
                        0x03, 0x06, 0x1a, 0x10, 0x05, 0x3a, 0x10, 0x00),
        CMD_VECT(0xfb,  0, 0x80, 0x0b, 0x12, 0x30, 0x37, 0x37, 0x36, 0x39,
                        0x0a, 0x12, 0x33, 0x27, 0x18, 0x3a, 0x10, 0x00),
        CMD_VECT(0xfc, 30, 0x80, 0x0b, 0x12, 0x30, 0x37, 0x37, 0x36, 0x39,
                        0x0a, 0x12, 0x33, 0x27, 0x18, 0x3a, 0x10, 0x00),

        CMD_VECT(0xf1,  0, 0x00, 0x00, 0x00, 0x00),
        CMD_VECT(0xf3, 30, 0x7f, 0x00, 0x00, 0x07, 0x33, 0x75, 0x66, 0x2c),
        CMD_VECT(0x3a,  0, 0x06, 0x00, 0x00, 0x00),
        CMD_VECT(0x36,  0, 0x48, 0x00, 0x00, 0x00),
        CMD_VECT(0xf2, 30, 0x1a, 0x1a, 0x03, 0x08, 0x08, 0x08, 0x08, 0x10,
			0x00, 0x17, 0x17, 0x00),
        CMD_VECT(0xe0,  0, 0x01, 0x00, 0x00, 0x00),
        CMD_VECT(0xe1,  0, 0xff, 0xf8, 0x00, 0x00),
        CMD_VECT(0x29,  0, 0x00, 0x00, 0x00, 0x00),
        CMD_VECT(0xcb,  0, 0x03, 0x00, 0x00, 0x00),
        CMD_VECT(0x55,  0, 0x03, 0x00, 0x00, 0x00),
        CMD_VECT(0xcd,  0, 0x4d, 0x15, 0x00, 0x00),
        CMD_VECT(0x35,  0, 0x00, 0x00, 0x00, 0x00),
};

struct lcm_va_cmd beect_auo_deinit_seq[] = {
	CMD_VECT(0xf3, 0, 0x80, 0x00, 0x00, 0x00),
	CMD_VECT(0x10, 30, 0x00, 0x00, 0x00, 0x00),
};

#define GPIOSEL_VWAKEINT (1U << 0)
#define INTMASK_VWAKEOUT (1U << 0)

enum {
	PANEL_TPO = 0,
	PANEL_AUO = 1,
};

#if defined(CONFIG_ARCH_MSM7225) || defined(CONFIG_ARCH_MSM7625)
#define LCM_ID0 32
#define LCM_ID1 23
#else
#define LCM_ID0 _bad_id()
#define LCM_ID1 _bad_id()
#endif

#define BR_USER_MIN	30
#define BR_USER_DEF	142
#define BR_USER_MAX	255
static int g_panel_type = PANEL_TPO;

static int
beect_panel_detect(void)
{
	int panel_id = 0;
	if(!gpio_get_value(LCM_ID0) && !gpio_get_value(LCM_ID1)) {
		panel_id = PANEL_TPO;
	} else if (gpio_get_value(LCM_ID0) && gpio_get_value(LCM_ID1)) {
		panel_id = PANEL_AUO;
	}

	return panel_id;
}

static int
beect_panel_shrink(int brightness)
{
	unsigned int pwm_max, pwm_def, pwm_min;

	if(g_panel_type == PANEL_TPO) {
		pwm_min = 6;
		pwm_def = 115;
		pwm_max = 255;
	} else {
		pwm_min = 5;
		pwm_def = 98;
		pwm_max = 217;
	}

	if (brightness < BR_USER_DEF) {
		if (brightness <= BR_USER_MIN)
			brightness = pwm_min;
		else
			brightness = (pwm_def-pwm_min)*(brightness-BR_USER_MIN)
					/(BR_USER_DEF-BR_USER_MIN) + pwm_min;
	} else {
		brightness = (pwm_max-pwm_def)*(brightness-BR_USER_DEF)
				/(BR_USER_MAX-BR_USER_DEF) + pwm_def;
	}
	return brightness;
}

static void
beect_process_mddi_table(struct msm_mddi_client_data *client_data,
		struct mddi_table *table, ssize_t count)
{
	int i;
	uint32_t reg, value, msec;

	BUG_ON(!client_data);
	BUG_ON(!table);
	BUG_ON(!count);

	for(i = 0; i < count; i++) {
		reg = table[i].reg;
		value = table[i].value;
		msec = table[i].msec;

		client_data->remote_write(client_data, value, reg);
		if (msec)
			hr_msleep(msec);
	}
}

static void
beect_process_mddi_cmd(struct msm_mddi_client_data *client_data,
                struct lcm_va_cmd *table, ssize_t count)
{
	struct lcm_va_cmd *pcmd = NULL;

	BUG_ON(!client_data);
	BUG_ON(!table);
	BUG_ON(!count);

	for (pcmd = table; pcmd < table + count; pcmd++) {
		client_data->remote_write_vals(client_data, pcmd->value,
			pcmd->reg, pcmd->size);
		if (pcmd->delay)
			hr_msleep(pcmd->delay);
	}
}

static void
beect_mddi_power_client(struct msm_mddi_client_data *cdata, int on)
{
	int ret;
	int vsync = 97;
	uint32_t config;
	B("%s: enter.\n", __func__);

	if(on) {
		B("beect resume\n");
		gpio_set_value(BEECT_LCMIO_2V6_EN, 1);
		hr_msleep(1);
		gpio_set_value(BEECT_LCM_2V85_EN, 1);
		hr_msleep(1);
		gpio_set_value(BEECT_MDDI_RSTz, 1);
		hr_msleep(1);
		config = PCOM_GPIO_CFG(vsync, 1, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA);
		ret = msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &config, 0);
	} else {
		B("beect suspend\n");
		gpio_set_value(BEECT_MDDI_RSTz, 0);
		gpio_set_value(BEECT_LCM_2V85_EN, 0);
		gpio_set_value(BEECT_LCMIO_2V6_EN, 0);
		config = PCOM_GPIO_CFG(vsync, 0, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_2MA);
		ret = msm_proc_comm(PCOM_RPC_GPIO_TLMM_CONFIG_EX, &config, 0);
	}

}

static int
beect_mddi_client_init(struct msm_mddi_bridge_platform_data *bridge_data,
		     struct msm_mddi_client_data *client_data)
{
	BUG_ON(!bridge_data);
	BUG_ON(!client_data);

	B("%s: enter. panel_id %d\n", __func__, g_panel_type);

	client_data->auto_hibernate(client_data, 0);
	if (g_panel_type == PANEL_TPO) {
		beect_process_mddi_cmd(client_data,
			beect_gamma_init_seq,
			ARRAY_SIZE(beect_gamma_init_seq));
		beect_process_mddi_table(client_data,
			beect_init_tb,
			ARRAY_SIZE(beect_init_tb));
	} else
		beect_process_mddi_cmd(client_data,
			beect_auo_init_seq,
			ARRAY_SIZE(beect_auo_init_seq));
	client_data->auto_hibernate(client_data, 1);

        return 0;

}

static int
beect_mddi_client_uninit(struct msm_mddi_bridge_platform_data *bridge_data,
			struct msm_mddi_client_data *client_data)

{
	BUG_ON(!bridge_data);
	BUG_ON(!client_data);

	B("%s: enter. panel_id %d\n", __func__, g_panel_type);

	client_data->auto_hibernate(client_data, 0);
	if(g_panel_type == PANEL_TPO)
		beect_process_mddi_table(client_data,
			beect_standby_tb,
			ARRAY_SIZE(beect_standby_tb));
	else
		beect_process_mddi_cmd(client_data,
			beect_auo_deinit_seq,
			ARRAY_SIZE(beect_auo_deinit_seq));
	client_data->auto_hibernate(client_data, 1);

        return 0;
}

static int
beect_panel_unblank(struct msm_mddi_bridge_platform_data *bridge_data,
                  struct msm_mddi_client_data *client_data)
{
	B("%s enter\n", __func__);
	if(g_panel_type == PANEL_TPO)
		client_data->remote_write(client_data, 0x24, 0x95);
	/*For AUO panel, the backlight control is called in mddi_eid_cabc.c. No need to add here*/
	return 0;
}

static int
beect_panel_blank(struct msm_mddi_bridge_platform_data *bridge_data,
                struct msm_mddi_client_data *client_data)
{
	B("%s enter\n", __func__);
	if(g_panel_type == PANEL_TPO)
		client_data->remote_write(client_data, 0x00, 0x95);
	/*For AUO panel, the backlight control is called in mddi_eid_cabc.c. No need to add here*/
	return 0;
}

static void
beect_fixup(uint16_t *mfr_name, uint16_t *product_code)
{
	B("%s: enter panel_id %d\n", __func__, g_panel_type);
	if(g_panel_type == PANEL_TPO) {
		*mfr_name = 0x4858;
		*product_code = 0x8356;
	} else {
		*mfr_name = 0x0101;
		*product_code = 0x04d1;
	}
}

static struct resource resources_msm_fb[] = {
        {
                .start = MSM_FB_BASE,
                .end = MSM_FB_BASE + MSM_FB_SIZE - 1,
                .flags = IORESOURCE_MEM,
        },
};

static struct msm_mddi_bridge_platform_data himax_client_data = {
        .init = beect_mddi_client_init,
        .uninit = beect_mddi_client_uninit,
	.bridge_type = SAMSUNG_D,
        .blank = beect_panel_blank,
        .unblank = beect_panel_unblank,
        .fb_data = {
                .xres = 240,
                .yres = 320,
		.width = 49,
		.height = 65,
                .output_format = 0,
        },
};

static struct msm_mddi_platform_data beect_pdata = {
	.clk_rate = 64000000,		/*default MDDI frequenct*/
	.power_client = beect_mddi_power_client,
	.fixup = beect_fixup,
	.fb_resource = resources_msm_fb,
	.num_clients = 2,
	.client_platform_data = {
		{
			.product_id = (0x4858 << 16 | 0x8356),
			.name = "mddi_c_4858_8356",
			.id = 0,
			.client_data = &himax_client_data,
			.clk_rate = 0,
		},
		{
			.product_id = (0x0101 << 16 | 0x04d1),
			.name = "mddi_c_0101_04d1",
			.id = 1,
			.client_data = &himax_client_data,
			.clk_rate = 0,
		},
	},
};

static struct msm_mdp_platform_data mdp_pdata = {
        .color_format = MSM_MDP_OUT_IF_FMT_RGB666,
};

int __init beect_panel_init(void)
{
	int rc;
	struct panel_data *config = &himax_client_data.panel_conf;

	msm_device_mdp.dev.platform_data = &mdp_pdata;

	g_panel_type = beect_panel_detect();
	B("beect panel_init id=%d", g_panel_type);

	rc = platform_device_register(&msm_device_mdp);
	if (rc)
		return rc;
	config->caps = MSMFB_CAP_CABC;
	config->shrink = 1;
	config->shrink_br = beect_panel_shrink;

	msm_device_mddi0.dev.platform_data = &beect_pdata;
	rc = platform_device_register(&msm_device_mddi0);
	if (rc)
		return rc;

	return 0;
}
