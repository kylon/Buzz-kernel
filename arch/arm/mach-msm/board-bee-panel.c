...
struct lcm_va_cmd bee_gamma_init_seq[] = {
CMD_VECT(0x5e, 0,
...
struct lcm_va_cmd bee_auo_init_seq[] = {
...
struct lcm_va_cmd bee_auo_deinit_seq[] = {
...
int g_panel_type = PANEL_TPO;

static int
bee_panel_detect(void)
{
	int panel_id = 0;
...
static int
bee_panel_shrink(int brightness)
{
...
static void
bee_process_mddi_cmd(struct msm_mddi_client_data *client_data,
		     ...
		     struct msm_mddi_client_data *client_data)
{
	...
	if (g_panel_type == PANEL_TPO) {
		bee_process_mddi_cmd(client_data,
			bee_gamma_init_seq,
			ARRAY_SIZE(bee_gamma_init_seq));
...
int __init bee_panel_init(void)
{
	...
	g_panel_type = bee_panel_detect();
	B("bee panel_init id=%d", g_panel_type);
	...
	config->shrink = 1;
	config->shrink_br = bee_panel_shrink;
